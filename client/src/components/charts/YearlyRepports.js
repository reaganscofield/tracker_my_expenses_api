...
export default class YearlyRepports extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    axios
      ...
      .then(res => {
        if (res.data) {
          const yearly = res.data.yearly;
          console.log('per year : ', yearly)
          if (yearly && typeof yearly === "object") {
            yearly.forEach(element => {
              if (this.state.labels.indexOf(moment(element.year).format("YYYY")) === -1) {
                this.state.labels.push(moment(element.year).format("YYYY"));
